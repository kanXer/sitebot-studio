...
export async function ingestAndBuildBot(params: IngestAndBuildParams): Promise<IngestAndBuildResult> {
  ...
  const synthesizedPrompt = [
    ...
    `- When appropriate or when the visitor expresses interest, ask for their full name, email address, AND phone number to follow up. Example: "I'd love to connect you with our team! Could you share your full name, email address, and phone number so we can reach out?"`,
    `- Always collect all three: name, email, and phone together in a single request for complete lead capture.`,
    `- For direct follow-up consultations or calls, confirm the phone number is reachable.`,
    `Specific Guardrails:`,
